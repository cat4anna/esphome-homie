...
homie::device_state HomieDevice::get_state() const {
  return device_state;
}
...
void HomieDevice::goto_state(homie::device_state new_state) {
  if(new_state != device_state) {
    ESP_LOGI(TAG, "State changed %s->%s", homie::enum_to_string(device_state).c_str(), homie::enum_to_string(new_state).c_str());
...
void HomieDevice::update() {
  ...
  if(last_known_state != app_state) {
    ...
    auto next = [&new_device_state](auto v){
      if(v > new_device_state) new_device_state = v;
    };
